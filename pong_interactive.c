...
int main(void) {
    ...
    cbreak();     // Не ждем enter
    noecho();     // Не показываем нажатые клавиши
    curs_set(0);  // Скрываем курсор
    timeout(0);   // getch() не должен ждать
    ...
    while (status) {
        ...
        napms(60);  // Задержка, независимая от ввода
    }
...
// Считывание команд с клавиатуры (теперь не блокирует игру)
int ReadCommand(void) {
    int sym = getch();
...
void PrintScreen(int left_score, int right_score, int left_paddle_y, int right_paddle_y, int ball_x,
                 int ball_y) {
    clear();  // Очистка экрана с помощью ncurses
    int row;
    int col;
    printw("\nPING PONG\n");
    printw("Player 1: %d | Player 2: %d\n", left_score, right_score);
    printw("Controls: A/Z and K/M, Q to quit\n\n");
...
void gameover(int left_score, int right_score) {
    ...
    timeout(-1);  // Останавливает изображение
    getch();      // Ждем ввода для завершения игры
}